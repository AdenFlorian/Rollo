...
public class TankMainGun : TankComponent {
	...
	public void Fire() {
		if (loadedShell != null) {
			...
			loadedShell.GetComponent<Rigidbody>().isKinematic = false;
			loadedShell.GetComponent<Rigidbody>().velocity = Vector3.zero;
			loadedShell.GetComponent<Rigidbody>().AddRelativeForce(0f, 0f, 50f, ForceMode.VelocityChange);
			loadedShell.GetComponent<Rigidbody>().AddRelativeTorque(0f, 0f, 10f, ForceMode.VelocityChange);
			...
			barrelTip.GetComponent<AudioSource>().Play();
			gunFireFX.GetComponent<ParticleSystem>().Play();
